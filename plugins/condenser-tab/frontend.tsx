...
function CondenserTab() {
  const condenser: any = (window as any).__condenser;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      const token = condenser?.core?.csrfToken;
      const fullUrl = token ? `${wsUrl}?auth=${token}` : wsUrl;
      ...
      socket.onmessage = (event: MessageEvent) => {
        ...
        if (data.type === 1 && data.result?.count !== undefined) {
  ...
  const send = (route: string) => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({ type: 0, route, id: ++callIdRef.current }));
    } else {
  ...
  return React.createElement(
    ...
    React.createElement(
      'button',
      { className: 'DialogButton _DialogLayout Secondary', onClick: () => send('click') },
