...
function CondenserTab({ React, websocketUrl }: { React: any; websocketUrl: string }) {
  const { count, send } = (window as any).__condenser.shared.useWebSocket(React, websocketUrl);

  const handleClick = () => send({ action: 'click' });
...
export default {
  ...
  key: 'condenser',
  title: 'Condenser',
  tab: (React: any) => React.createElement(
    'svg',
